...
function stripSiteSuffix(value){return clean(value).replace(/\s+[-–—]\s+(?:bus\s*times|bustimes(?:\.org)?).*$/i,'').trim();}
function parseOfficialTitle(text){
 const lines=String(text||'').split(/\r?\n/).map(clean).filter(Boolean);
 const heading=lines.find(x=>/^#\s+/.test(x)&&!/bustimes\.org/i.test(x));
 if(heading)return stripSiteSuffix(heading.replace(/^#\s+/,''));
 const title=lines.find(x=>/^Title:\s*/i.test(x));
 if(title)return stripSiteSuffix(title.replace(/^Title:\s*/i,''));
 return'';
}
...
async function locate(showModal){currentStops=[];renderStops();status('Finding location and matching official stop names…');if(showModal)$('nearbyStopsModal')?.classList.add('show');try{currentStops=await sharedLookup(true);renderStops();if(currentStops.length){if(!$('csBoarding')?.value)setBoarding(currentStops[0]);status(showModal?'Nearest stop selected. Tap another stop if required.':'Nearest bus stop added automatically.');const use=$('useCurrentLocationBtn');if(use)use.style.display='none';}else fallback();}catch(e){fallback();}}
