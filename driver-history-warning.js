...
function render(){const d=$('csDriver'),w=ensure();if(!d||!w)return;const name=d.value.trim();if(!name){w.className='';w.innerHTML='';return;}const seen=new Set();const hits=inspectionRows().map(r=>({type:typeOf(r[13]),date:r[0],driver:r[4],reason:r[14]})).filter(x=>{if(!x.type||!same(x.driver,name))return false;const key=[clean(x.driver),String(x.date||''),x.type,clean(x.reason)].join('|');if(seen.has(key))return false;seen.add(key);return true;});if(!hits.length){w.className='';w.innerHTML='';return;}hits.reverse();const reported=hits.some(x=>x.type==='reported');w.className='show '+(reported?'reported':'advised');w.innerHTML='<div class="t">'+(reported?'⚠️ PREVIOUS REPORTED ISSUE':'⚠️ PREVIOUS ADVICE RECORDED')+'</div>'+hits.map(x=>'<div class="item"><b>'+(x.type==='reported'?'Reported':'Advised')+' — '+esc(x.date||'Date not recorded')+'</b><div class="reason">'+esc(x.reason||'No offence or advisory reason entered')+'</div><button type="button" class="viewFullReport" data-history-driver="'+esc(name)+'">VIEW FULL REPORT</button></div>').join('');}
function init(){addStyle();ensure();const d=$('csDriver');if(!d)return;['input','change','blur'].forEach(ev=>d.addEventListener(ev,()=>setTimeout(render,100)));document.addEventListener('click',e=>{if(e.target.closest('[data-driver-suggestion]'))setTimeout(render,180);const b=e.target.closest('.viewFullReport');if(b){e.preventDefault();e.stopPropagation();openFullReport(b.dataset.historyDriver||d.value);}});[1200,3000,6000].forEach(ms=>setTimeout(render,ms));}
