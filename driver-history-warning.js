...
function inspectionRows(){
 try{
  if(typeof cloud!=='undefined'&&cloud&&Array.isArray(cloud['Inspections'])&&cloud['Inspections'].length){
   return stripHeader(cloud['Inspections'].slice());
  }
 }catch(e){}
 try{
  const local=JSON.parse(localStorage.getItem('local_Inspections')||'[]');
