...
async function makePdf(d,name){
 ...
 function tableRow(label,value){const labelW=190;const labelText=String(label||'-');const valueText=String(value||'-');pdf.setFont('helvetica','bold');pdf.setFontSize(10.5);const labelLines=pdf.splitTextToSize(labelText,labelW-20);pdf.setFont('helvetica','normal');pdf.setFontSize(11);const valueLines=pdf.splitTextToSize(valueText,contentW-labelW-22);const lineHeight=13,topPad=11,bottomPad=8;const labelHeight=topPad+(labelLines.length*lineHeight)+bottomPad;const valueHeight=topPad+(valueLines.length*lineHeight)+bottomPad;const h=Math.max(31,labelHeight,valueHeight);ensure(h);fill(C.light);pdf.rect(left,y,labelW,h,'F');fill(C.white);pdf.rect(left+labelW,y,contentW-labelW,h,'F');pdf.setDrawColor(C.line[0],C.line[1],C.line[2]);pdf.rect(left,y,contentW,h,'S');pdf.line(left+labelW,y,left+labelW,y+h);colour(C.label);pdf.setFont('helvetica','bold');pdf.setFontSize(10.5);pdf.text(labelLines,left+10,y+18);colour(C.text);pdf.setFont('helvetica','normal');pdf.setFontSize(11);pdf.text(valueLines,left+labelW+10,y+18);y+=h;}
