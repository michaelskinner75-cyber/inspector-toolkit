...
function reportRows(){
 ...
 const date=$('mroDate')?.value||'',depot=selectedDepot();
 return [...map.values()].filter(r=>toIso(r[0])===date&&(!depot||norm(depotOf(r))===norm(depot))).sort((a,b)=>((mins(a[8])??9999)-(mins(b[8])??9999))||((mins(a[1])??9999)-(mins(b[1])??9999)));
}
...
async function createPdf(){
 const depot=selectedDepot(),rows=reportRows();if(!rows.length){alert('There are no Morning Run Out entries for this date'+(depot?' and depot':'')+'.');return;}
 ...
 try{
  ...
  const date=$('mroDate')?.value||'',inspector=typeof getInspector==='function'?getInspector():'-',reportDepot=depot||'All Depots',checked=rows.filter(r=>diff(r[8],r[9])!==null),late=checked.filter(r=>diff(r[8],r[9])>=6).length,on=checked.length-late,score=checked.length?on/checked.length*100:0;
  const meta=[['DATE',uk(date)],['DEPOT',reportDepot],['INSPECTOR',inspector||'-'],['BUSES CHECKED',String(checked.length)],['ON TIME / WITHIN 5',String(on)],['6+ MIN LATE',String(late)],['RUN OUT SCORE',score.toFixed(1)+'%']];const mw=contentW/meta.length;fill(C.light);pdf.rect(left,y,contentW,42,'F');pdf.setDrawColor(...C.line);pdf.rect(left,y,contentW,42,'S');meta.forEach((m,i)=>{const x=left+i*mw;if(i)pdf.line(x,y,x,y+42);colour(C.muted);pdf.setFont('helvetica','normal');pdf.setFontSize(6.5);pdf.text(m[0],x+6,y+13);colour(C.text);pdf.setFont('helvetica','bold');pdf.setFontSize(10.5);pdf.text(String(m[1]),x+6,y+30);});y+=54;
  tableHeader();rows.forEach(row);footer();
  pdf.setProperties({title:`Morning Run Out Report ${reportDepot} ${uk(date)}`,subject:'Morning Run Out Check Report',author:'Stagecoach South Scotland'});
  const safeDepot=reportDepot.replace(/[^a-z0-9]+/gi,'-'),bytes=pdf.output('arraybuffer'),name=`Morning-Run-Out-Report-${safeDepot}-${date||'Report'}.pdf`,file=new File([bytes],name,{type:'application/pdf',lastModified:Date.now()});
