...
const MODES={phone:'Standard Phone',plus:'Plus Phone',large:'iPad / Laptop'};
function chooseDefault(){const w=Math.max(document.documentElement.clientWidth||0,window.innerWidth||0);if(w>=760)return'large';if(w>=430)return'plus';return'phone';}
function apply(mode){if(!MODES[mode])mode=chooseDefault();document.body.classList.remove('display-phone','display-plus','display-large');document.body.classList.add('display-'+mode);document.documentElement.dataset.displaySize=mode;localStorage.setItem(KEY,mode);document.querySelectorAll('[data-display-size]').forEach(b=>{const on=b.dataset.displaySize===mode;b.classList.toggle('selected',on);b.setAttribute('aria-pressed',String(on));});}
function addStyles(){if(document.getElementById('displaySizeCss'))return;const s=document.createElement('style');s.id='displaySizeCss';s.textContent=`
...
.displaySizeChoices{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}
...
body.display-large .wrap{max-width:1240px;padding:14px}
body.display-large .machine{padding:16px}
body.display-large .nav{grid-template-columns:repeat(3,1fr)!important;gap:12px}
body.display-large .grid{grid-template-columns:repeat(3,1fr)!important;gap:11px}
body.display-large .grid2{grid-template-columns:repeat(2,1fr)!important;gap:11px}
body.display-large .nav button{min-height:78px;font-size:16px}
body.display-large .panel,body.display-large .log{padding:16px}
body.display-large .field,body.display-large .searchBar{font-size:17px;padding:14px}
body.display-large .brandTitle{font-size:32px}
body.display-large .serviceFrame{height:760px}
@media(max-width:480px){body.display-large .nav,body.display-large .grid,body.display-large .grid2{grid-template-columns:1fr!important}}
`;document.head.appendChild(s);}
function build(){const card=document.querySelector('#loginPage .loginCard');if(!card||document.getElementById('displaySizeBox'))return;const version=document.getElementById('loginVersion');const box=document.createElement('div');box.id='displaySizeBox';box.className='displaySizeBox';box.innerHTML='<div class="displaySizeTitle">Choose screen size</div><div class="displaySizeChoices"><button type="button" class="displaySizeChoice" data-display-size="phone"><span class="sizeIcon">📱</span>Standard Phone</button><button type="button" class="displaySizeChoice" data-display-size="plus"><span class="sizeIcon">📲</span>Plus Phone</button><button type="button" class="displaySizeChoice" data-display-size="large"><span class="sizeIcon">💻</span>iPad / Laptop</button></div><div class="displaySizeHint">Your choice is remembered for next time.</div>';
