...
function openManagementFromTools(){
 ...
 if(typeof loadCloud==='function'){
  const p=loadCloud();
  if(p&&typeof p.then==='function')p.then(()=>setTimeout(()=>$('mgRefresh')?.click(),50));
  else setTimeout(()=>$('mgRefresh')?.click(),900);
 }
 window.scrollTo(0,0);
}
function setup(){
 ...
 if(!btn){
  ...
  btn.type='button';
  btn.innerHTML='<span class="navIcon">📊</span>Management Overview Summary';
  nav.appendChild(btn);
 }
 btn.onclick=openManagementFromTools;
 const back=$('mgBack');
 if(back&&back.dataset.toolBack!=='1'){
  back.dataset.toolBack='1';
  back.textContent='← Other Links & Tools';
  back.addEventListener('click',e=>{
   e.preventDefault();
   e.stopImmediatePropagation();
   if(typeof openSection==='function')openSection('otherLinksTools');
  },true);
 }
}
function init(){
 ...
 document.addEventListener('click',e=>{
  if(e.target.closest('#loginBtn,[data-login-name],#changeInspectorBtn'))setTimeout(setup,250);
 });
}
