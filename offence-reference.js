(function(){
 ...
 window.saveCheckSheet=function(){
  ...
  const row=[date,savedTime,inspector,$('csDepot').value,$('csDriver').value,$('csService').value,$('csFleet').value,normaliseTimeForSave($('csTimeOn').value),$('csBoarding').value,$('csDestination').value,$('csNSA').value,$('csNSAFault').value,$('csNSANotes').value,reportType,$('csDriverReason').value,offenceRef];
  ...
  if(reportType!=='No Driver Report'){
   cloudAppend('Driver Reports',[date,savedTime,inspector,$('csDriver').value,$('csDepot').value,reportType,$('csDriverReason').value,$('csService').value,$('csFleet').value,offenceRef]);
  }
  ...
  if(reportType!=='No Driver Report'){
   ...
   cloud['Driver Reports'].push([date,savedTime,inspector,$('csDriver').value,$('csDepot').value,reportType,$('csDriverReason').value,$('csService').value,$('csFleet').value,offenceRef]);
  }
 ...
 window.renderChecks=function(){
  ...
  $('checkList').innerHTML=rows.reverse().map((r,i)=>{
   ...
   const details=[ref?'<b>Offence Reference: '+ref+'</b>':'',`${date} ${savedTime}`,r[2]||'-',r[3]||'-',r[4]||'-',r[5]||'-',r[6]||'-',timeOn,`${r[8]||'-'} to ${r[9]||'-'}`,`NSA: ${r[10]||'-'}${r[10]==='No'?' - '+(r[11]||'-')+' - '+(r[12]||'-'):''}`,`Driver Report: ${r[13]||'-'}`,r[14]||'-'].filter(Boolean);
