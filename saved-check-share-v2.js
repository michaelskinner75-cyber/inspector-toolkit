...
function parseExtraDetails(lines){
 const joined=lines.join('\n').trim();
 let vehicleIssue='-',vehicleComments='-',driverNotes=joined||'-';
 const issueMatch=joined.match(/Vehicle\s+issue\s*:\s*([^\n]*?)(?=\s+Vehicle\s+comments\s*:|\n|$)/i);
 const commentsMatch=joined.match(/Vehicle\s+comments\s*:\s*([\s\S]*?)(?=\n(?:Driver\s+(?:report\s+)?reason|Inspection\s+notes)\s*:|$)/i);
 if(issueMatch)vehicleIssue=issueMatch[1].trim()||'-';
 if(commentsMatch)vehicleComments=commentsMatch[1].trim()||'-';
 driverNotes=joined
  .replace(/Vehicle\s+issue\s*:\s*[^\n]*?(?=\s+Vehicle\s+comments\s*:|\n|$)/ig,'')
  .replace(/Vehicle\s+comments\s*:\s*[\s\S]*?(?=\n(?:Driver\s+(?:report\s+)?reason|Inspection\s+notes)\s*:|$)/ig,'')
  .replace(/^(Driver\s+report\s+reason\s*\/\s*inspection\s+notes|Driver\s+(?:report\s+)?reason|Inspection\s+notes|Reason|Notes)\s*:\s*/i,'')
  .trim()||'-';
 return{vehicleIssue,vehicleComments,driverNotes};
}
function reportFromCard(card){
 const x=linesFromCard(card),journey=parseJourney(x[7]),nsaRaw=strip(x[8],['NSA']),nsaParts=nsaRaw.split(/\s+-\s+/),dt=splitDateTime(strip(x[0],['Inspection date/time','Date']));
 const extras=parseExtraDetails(x.slice(10));
 return{date:dt.date,savedTime:dt.time,inspector:strip(x[1],['Inspector']),depot:strip(x[2],['Depot']),driver:strip(x[3],['Driver']),service:strip(x[4],['Service']),fleet:strip(x[5],['Fleet Number','Fleet']),timeChecked:strip(x[6],['Time Checked','Time boarded']),boarding:journey.boarding,destination:journey.destination,nsa:strip(nsaParts[0],['NSA Working']),nsaDetails:strip(nsaParts.slice(1).join(' - ')||((nsaParts[0]||'')==='Yes'?'Fully Working':'-'),['Details']),driverReport:strip(x[9],['Driver Report']),reason:extras.driverNotes,vehicleIssue:extras.vehicleIssue,vehicleComments:extras.vehicleComments};
}
...
async function makePdf(d,name){
 ...
 sectionTitle('Driver');tableRow('Driver Report',d.driverReport);tableRow('Full driver report reason / inspection notes',d.reason);
 if(d.vehicleIssue!=='-'||d.vehicleComments!=='-'){y+=14;sectionTitle('Vehicle Faults');tableRow('Vehicle issue',d.vehicleIssue);tableRow('Vehicle comments',d.vehicleComments);}
...
function decorate(){document.querySelectorAll('#checkList .compactCheck').forEach(card=>{let old=card.querySelector('.savedCheckShareBtn');if(old){const replacement=old.cloneNode(true);old.replaceWith(replacement);replacement.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();share(card);});return;}const btn=document.createElement('button');btn.type='button';btn.className='savedCheckShareBtn';btn.textContent='SEND REPORT';btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();share(card);});const top=card.querySelector('.compactTop');(top||card).appendChild(btn);});}
function removeLive(){const c=document.getElementById('checksheetShareControls');if(c)c.remove();}
function style(){if(document.getElementById('savedCheckShareCssV2'))return;const s=document.createElement('style');s.id='savedCheckShareCssV2';s.textContent='.compactTop{position:relative;padding-bottom:46px!important}.savedCheckShareBtn{position:absolute;right:12px;bottom:8px;border:1px solid #f4b23f;border-radius:9px;background:#f3aa35;color:#07131e;font-weight:800;font-size:12px;padding:8px 12px;z-index:3}.savedCheckShareBtn:disabled{opacity:.7}.savedCheckShareBtn:active{transform:scale(.98)}';document.head.appendChild(s);}
