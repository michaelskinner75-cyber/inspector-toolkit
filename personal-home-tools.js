...
function renderHome(){
 ...
 const html=ids.map(id=>'<button data-open="'+id+'"><span class="navIcon">⭐</span>'+titleFor(id)+'</button>').join('')||'<div class="myHomeToolsEmpty">Open any tool and press “Add to Home Screen”.</div>';
 if(dest.innerHTML!==html)dest.innerHTML=html;
}
function addControls(){
 allTools().forEach(id=>{const section=$(id);if(!section||section.querySelector('.homeToolToggle'))return;const btn=document.createElement('button');btn.type='button';btn.className='btn homeToolToggle';btn.dataset.homeTool=id;const back=section.querySelector('.backBtn');if(back)back.insertAdjacentElement('afterend',btn);else section.insertBefore(btn,section.firstChild);});
 updateControls();
}
function updateControls(){const set=new Set(saved());document.querySelectorAll('.homeToolToggle').forEach(btn=>{const on=set.has(btn.dataset.homeTool);const label=on?'✓ Remove from Home Screen':'＋ Add to Home Screen';if(btn.textContent!==label)btn.textContent=label;btn.classList.toggle('activeHomeTool',on);});}
...
function refresh(){addControls();renderHome();}
function init(){
 refresh();
 ...
 const name=$('loggedInInspector');if(name)new MutationObserver(()=>{renderHome();updateControls();}).observe(name,{childList:true,subtree:true,characterData:true});
 setTimeout(refresh,1200);setTimeout(refresh,3000);
}
